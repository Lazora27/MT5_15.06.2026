...
int OnInit()
  {
   trade.SetExpertMagicNumber(1031);
...
void OnTick()
  {
   ...
   if(CopyClose(_Symbol, _Period, 1, 5, close_buffer) <= 0) return;
   ...
   for (int i = 0; i <= 2; i++)
     {
      double currentReturn = close_buffer[i] - close_buffer[i + 1];
      momentumSum += currentReturn;
     }
   double momentum = momentumSum / 3.0;

   double smaVal = sma_buffer[0];
   double deviation = close - smaVal;

   bool isForecastBullish = (momentum > 0) && (MathAbs(deviation) < atr_buffer[0] * 2);
   bool isTrendUp = close > smaVal;

   if (isForecastBullish && isTrendUp && !HasOpenPosition())
     {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 1031)
        {
         return true;
        }
     }
   ...
   if(type == ORDER_TYPE_BUY)
     {
      ...
      trade.Buy(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "VARForecast_031");
     }
  }
...
string GetStrategyName() { return "VAR_Forecast"; }
