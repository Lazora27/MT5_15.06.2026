...
// --- 1. PARAMETER ---
input int InpPeriod = 7;             // DMI Periode
input double ADXThreshold = 15.0;    // ADX Schwellenwert
input int    EmaInpPeriod = 50;         // Trend Filter (EMA)
...
double         adx_plus_buffer[];
double         adx_minus_buffer[];
double         ema_buffer[];
double         close_buffer[];
...
int OnInit()
  {
   ...
   adx_handle = iADX(_Symbol, _Period, InpPeriod);
   if(adx_handle == INVALID_HANDLE) return(INIT_FAILED);

   ema_handle = iMA(_Symbol, _Period, EmaInpPeriod, 0, MODE_EMA, PRICE_CLOSE);
...
void OnTick()
  {
   ...
   if(CopyBuffer(adx_handle, 0, 1, 2, adx_main_buffer) <= 0) return;
   if(CopyBuffer(adx_handle, 1, 1, 2, adx_plus_buffer) <= 0) return;
   if(CopyBuffer(adx_handle, 2, 1, 2, adx_minus_buffer) <= 0) return;
   if(CopyBuffer(ema_handle, 0, 1, 1, ema_buffer) <= 0) return;
   if(CopyClose(_Symbol, _Period, 1, 1, close_buffer) <= 0) return;

   last_time = current_time;

   double plusDi = adx_plus_buffer[0];
   double minusDi = adx_minus_buffer[0];
   double prevPlusDi = adx_plus_buffer[1];
   double prevMinusDi = adx_minus_buffer[1];
   double adx = adx_main_buffer[0];
   double close = close_buffer[0];
   double ema = ema_buffer[0];

   bool crossUp = (prevPlusDi <= prevMinusDi) && (plusDi > minusDi);
   bool strongTrendLong = adx > ADXThreshold;
   bool trendConfirmLong = close > ema;
   bool longSignal = crossUp && strongTrendLong && trendConfirmLong;

   bool crossDown = (prevMinusDi <= prevPlusDi) && (minusDi > plusDi);
   bool strongTrendShort = adx > ADXThreshold;
   bool trendConfirmShort = close < ema;
   bool shortSignal = crossDown && strongTrendShort && trendConfirmShort;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 1004)
        {
   ...
   if(type == ORDER_TYPE_BUY)
     {
      ...
      trade.Buy(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "DMI_004");
     }
   else
     {
      slPrice = currentPrice + (slPips * pipSize);
      tpPrice = currentPrice - (tpPips * pipSize);
      trade.Sell(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "DMI_004");
     }
  }
