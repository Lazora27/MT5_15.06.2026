...
// --- 1. PARAMETER ---
input int InpPeriod = 21;            // Indikator Periode
...
int OnInit()
  {
   trade.SetExpertMagicNumber(1013);
...
void OnTick()
  {
   ...
   if(CopyBuffer(macd_handle, 1, 1, 2, macd_signal_buffer) <= 0) return;
   if(CopyClose(_Symbol, _Period, 1, 3, close_buffer) <= 0) return;

   last_time = current_time;

   double close1 = close_buffer[0];
   double close2 = close_buffer[1];

   int signals = 0;
   const double total = 4.0;

   if(close1 > sma_buffer[0])
     {
      signals++;
     }
   ...
   double metaScore = signals / total;

   if(metaScore >= 0.75 && !HasOpenPosition())
     {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 1013)
        {
         return true;
        }
     }
   ...
   if(type == ORDER_TYPE_BUY)
     {
      ...
      trade.Buy(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "MetaLearning_013");
     }
  }
...
string GetStrategyName() { return "Meta_Learning"; }
