...
// --- 1. PARAMETER ---
input int InpPeriod = 10;            // Indikator Periode
...
void OnTick()
  {
   ...
   if(CopyBuffer(atr_handle, 0, 1, copy_count, atr_buffer) <= 0) return;
   ...
   for (int i = 0; i < 5; i++)
     {
      double prevClose = close_buffer[i + 1];
      if (prevClose > 0)
        {
         double ret = (close_buffer[i] - prevClose) / prevClose;
         momentum += ret;
        }
     }

   double atrVal = atr_buffer[0];
   double atrSum = 0;
   for (int i = 0; i < InpPeriod; i++)
     {
      atrSum += atr_buffer[i];
     }
   double atrAvg = atrSum / InpPeriod;

   bool isQualityOk = atrVal < (atrAvg * 1.5);

   bool isDirectionPositive = momentum > 0;
   bool isTrendUp = close > smaVal;

   if (isQualityOk && isDirectionPositive && isTrendUp && !HasOpenPosition())
     {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 1014)
        {
         return true;
        }
     }
   ...
   if(type == ORDER_TYPE_BUY)
     {
      ...
      trade.Buy(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "NonlinearPrediction_014");
     }
  }
...
string GetStrategyName() { return "Nonlinear_Prediction"; }
