...
// --- 1. PARAMETER ---
input int InpPeriod = 5;             // Haupt-Periode (SMA/BB/Stoch)
input double Threshold = 0.6;        // Ensemble Schwelle
...
int OnInit()
  {
   trade.SetExpertMagicNumber(1023);
   ...
   stoch_handle = iStochastic(_Symbol, _Period, InpPeriod, 3, 3, MODE_SMA, STO_LOWHIGH);
   bb_handle = iBands(_Symbol, _Period, InpPeriod, 0, 2.0, PRICE_CLOSE);

   if(sma_handle == INVALID_HANDLE || rsi_handle == INVALID_HANDLE || macd_handle == INVALID_HANDLE || stoch_handle == INVALID_HANDLE || bb_handle == INVALID_HANDLE) 
      return(INIT_FAILED);
   ...
   ArraySetAsSeries(bb_bottom_buffer, true);
...
void OnTick()
  {
   ...
   if(CopyBuffer(macd_handle, 0, 1, 2, macd_main_buffer) <= 0) return;
   if(CopyBuffer(stoch_handle, 0, 1, 2, stoch_k_buffer) <= 0) return;
   if(CopyBuffer(bb_handle, 2, 1, 2, bb_bottom_buffer) <= 0) return; // 2 = Lower band
   ...
   int votes = 0;
   const double totalVotes = 10.0;

   double close = close_buffer[0];

   if (close > sma_buffer[0]) votes++;
   if (rsi_buffer[0] < 70) votes++;
   if (macd_main_buffer[0] > 0) votes++;
   if (close > close_buffer[5]) votes++; // Last(6) => index 5
   if (stoch_k_buffer[0] < 80) votes++;
   if (close > bb_bottom_buffer[0]) votes++;
   if (high_buffer[0] > high_buffer[1]) votes++;
   if (low_buffer[0] > low_buffer[1]) votes++;
   if (close > open_buffer[0]) votes++;
   if (rsi_buffer[0] > 30) votes++;

   double ensembleScore = votes / totalVotes;

   if (ensembleScore > Threshold && !HasOpenPosition())
     {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 1023)
        {
         return true;
        }
     }
   ...
   if(type == ORDER_TYPE_BUY)
     {
      ...
      trade.Buy(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "RandomForest_023");
     }
  }
