...
int OnInit()
  {
   trade.SetExpertMagicNumber(1021);
...
void OnTick()
  {
   ...
   if (atrVal > 0)
     {
      vigor = (close - smaVal) / atrVal;
     }

   bool trendUp = smaVal > smaPrev;
   bool strongVigor = MathAbs(vigor) > 0.5;
   bool positiveVigor = vigor > 0;

   if (strongVigor && positiveVigor && trendUp && !HasOpenPosition())
     {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 1021)
        {
         return true;
        }
     }
   ...
   if(type == ORDER_TYPE_BUY)
     {
      ...
      trade.Buy(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "TrendVigor_021");
     }
  }
...
string GetStrategyName() { return "Trend_Vigor"; }
