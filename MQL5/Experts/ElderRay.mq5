...
// --- 1. PARAMETER ---
input int InpPeriod = 5;             // EMA Periode
...
int OnInit()
  {
   trade.SetExpertMagicNumber(1011);

   ema_handle = iMA(_Symbol, _Period, InpPeriod, 0, MODE_EMA, PRICE_CLOSE);
...
void OnTick()
  {
   ...
   if(CopyBuffer(ema_handle, 0, 1, 2, ema_buffer) <= 0) return;
   if(CopyHigh(_Symbol, _Period, 1, 2, high_buffer) <= 0) return;
   if(CopyLow(_Symbol, _Period, 1, 2, low_buffer) <= 0) return;
   ...
   double bullPower = high - emaValue;
   double bearPower = low - emaValue;

   double prevEma = ema_buffer[1];
   double prevLow = low_buffer[1];
   
   double prevBearPower = prevLow - prevEma;

   bool entrySignal = (bullPower > 0) && (bearPower > prevBearPower);

   if (entrySignal && !HasOpenPosition())
     {
      ExecuteOrder(ORDER_TYPE_BUY); // Nur Buy-Logik im cTrader Code für diese Strategie? Ah, das original hat nur Buy.
     }
  }
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 1011)
        {
         return true;
        }
     }
   ...
   if(type == ORDER_TYPE_BUY)
     {
      ...
      trade.Buy(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "ElderRay_011");
     }
   else
     {
      slPrice = currentPrice + (slPips * pipSize);
      tpPrice = currentPrice - (tpPips * pipSize);
      trade.Sell(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "ElderRay_011");
     }
  }
...
string GetStrategyName() { return "ElderRay"; }
