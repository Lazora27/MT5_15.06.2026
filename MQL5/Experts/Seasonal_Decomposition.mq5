...
// --- 1. PARAMETER ---
input int InpPeriod = 5;             // Indikator Basis-Periode
...
int OnInit()
  {
   trade.SetExpertMagicNumber(1029);

   sma_handle = iMA(_Symbol, _Period, InpPeriod, 0, MODE_SMA, PRICE_CLOSE);
   sma_long_handle = iMA(_Symbol, _Period, InpPeriod * 2, 0, MODE_SMA, PRICE_CLOSE);
   
   if(sma_handle == INVALID_HANDLE || sma_long_handle == INVALID_HANDLE) 
      return(INIT_FAILED);

   ArraySetAsSeries(sma_buffer, true);
   ArraySetAsSeries(sma_long_buffer, true);
...
void OnDeinit(const int reason)
  {
   ...
   IndicatorRelease(sma_long_handle);
  }
...
void OnTick()
  {
   ...
   if(CopyBuffer(sma_long_handle, 0, 1, 1, sma_long_buffer) <= 0) return;
   ...
   double smaPrev = sma_buffer[1];

   double trendSignal = (smaVal > smaPrev) ? 1.0 : 0.0;

   double deviation = close - smaVal;
   double prevDeviation = close_buffer[1] - smaPrev;

   double seasonalSignal = (deviation > prevDeviation || deviation > 0) ? 1.0 : 0.0;

   const double totalSignals = 2.0;
   double combinedScore = (trendSignal + seasonalSignal) / totalSignals;

   if (combinedScore > 0.5 && !HasOpenPosition())
     {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 1029)
        {
         return true;
        }
     }
   ...
   if(type == ORDER_TYPE_BUY)
     {
      ...
      trade.Buy(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "SeasonalDecomp_029");
     }
  }
...
string GetStrategyName() { return "Seasonal_Decomposition"; }
