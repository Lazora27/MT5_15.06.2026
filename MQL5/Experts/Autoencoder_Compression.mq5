...
// --- 1. PARAMETER ---
input int InpPeriod = 5;             // Analyse-Periode
...
double         bb_top_buffer[];
double         bb_bot_buffer[];
double         sma_buffer[];
double         close_buffer[];
...
int OnInit()
  {
   ...
   if(bb_handle == INVALID_HANDLE)
     {
      Print("Fehler beim Erstellen des BB Indikators");
      return(INIT_FAILED);
     }
   ...
   if(sma_handle == INVALID_HANDLE)
     {
      Print("Fehler beim Erstellen des SMA Indikators");
      return(INIT_FAILED);
     }

   ArraySetAsSeries(bb_top_buffer, true);
   ArraySetAsSeries(bb_bot_buffer, true);
...
void OnTick()
  {
   ...
   if(CopyBuffer(bb_handle, 1, 1, 1, bb_top_buffer) <= 0) return; // UPPER
   if(CopyBuffer(bb_handle, 2, 1, 1, bb_bot_buffer) <= 0) return; // LOWER
   ...
   double bbTop = bb_top_buffer[0];
   double bbBot = bb_bot_buffer[0];

   double distanceLong = MathAbs(close - bbBot) / close;
   bool compressionLong = distanceLong < 0.003;
   bool uptrend = close > smaVal;
   bool longSignal = compressionLong && uptrend;

   double distanceShort = MathAbs(close - bbTop) / close;
   bool compressionShort = distanceShort < 0.003;
   bool downtrend = close < smaVal;
   bool shortSignal = compressionShort && downtrend;
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 1007)
        {
   ...
   if(type == ORDER_TYPE_BUY)
     {
      ...
      trade.Buy(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "AE_007");
     }
   else
     {
      slPrice = currentPrice + (slPips * pipSize);
      tpPrice = currentPrice - (tpPips * pipSize);
      trade.Sell(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "AE_007");
     }
  }
...
string GetStrategyName() { return "Autoencoder_Compression"; }
