...
input double Threshold = 25.0;       // Info Share Schwelle
...
int OnInit()
  {
   trade.SetExpertMagicNumber(1012);
...
void OnTick()
  {
   ...
   if(CopyBuffer(atr_handle, 0, 1, 2, atr_buffer) <= 0) return;
   if(CopyClose(_Symbol, _Period, 1, 3, close_buffer) <= 0) return;

   last_time = current_time;

   double close1 = close_buffer[0]; // Last(1)
   double close2 = close_buffer[1]; // Last(2)
   
   double smaVal = sma_buffer[0];
   double atrVal = atr_buffer[0];

   double priceChange = MathAbs(close1 - close2);
   double infoShare = 0;

   if(atrVal > 0)
     {
      infoShare = (priceChange / atrVal) * 100.0;
     }

   bool isBullish = close1 > close2;
   bool isTrendUp = close1 > smaVal;

   if(infoShare > Threshold && isBullish && isTrendUp && !HasOpenPosition())
     {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 1012)
        {
         return true;
        }
     }
   ...
   if(type == ORDER_TYPE_BUY)
     {
      ...
      trade.Buy(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "Hasbrouck_012");
     }
  }
...
string GetStrategyName() { return "Hasbrouck_Info_Share"; }
