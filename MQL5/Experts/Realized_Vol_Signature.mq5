...
// --- 1. PARAMETER ---
input int InpPeriod = 5;             // Indikator Periode
...
void OnTick()
  {
   ...
   double volRatio = (atrAvg > 0) ? (atrCurrent / atrAvg) : 1.0;

   bool isLowVol = volRatio < 1.0;

   double smaVal = sma_buffer[0];
   bool isTrendUp = close > smaVal;
   bool isMomentumUp = close > close_buffer[1];

   if (isLowVol && isTrendUp && isMomentumUp && !HasOpenPosition())
     {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 1034)
        {
         return true;
        }
     }
   ...
   if(type == ORDER_TYPE_BUY)
     {
      ...
      trade.Buy(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "RealVolSig_034");
     }
  }
...
string GetStrategyName() { return "Realized_Vol_Signature"; }
