...
// --- 1. PARAMETER ---
input int InpPeriod = 2;             // Vortex Periode
...
int OnInit()
  {
   trade.SetExpertMagicNumber(1005);
...
bool CalculateVortex(int offset, double &viPlus, double &viMinus)
  {
   double vmPlusSum = 0;
   double vmMinusSum = 0;
   double trSum = 0;
   ...
   for (int i = 0; i < InpPeriod; i++)
     {
      ...
      double high = high_buffer[idx];
      double low = low_buffer[idx];
      
      double prevHigh = high_buffer[idx + 1];
      double prevLow = low_buffer[idx + 1];
      double prevClose = close_buffer[idx + 1];

      vmPlusSum += MathAbs(high - prevLow);
      vmMinusSum += MathAbs(low - prevHigh);
   ...
   viPlus = vmPlusSum / trSum;
   viMinus = vmMinusSum / trSum;
...
void OnTick()
  {
   ...
   int copy_count = InpPeriod + 5;
   if(CopyHigh(_Symbol, _Period, 0, copy_count, high_buffer) <= 0) return;
   if(CopyLow(_Symbol, _Period, 0, copy_count, low_buffer) <= 0) return;
   if(CopyClose(_Symbol, _Period, 0, copy_count, close_buffer) <= 0) return;

   last_time = current_time;

   double viPlus, viMinus;
   double prevViPlus, prevViMinus;

   if (!CalculateVortex(1, viPlus, viMinus)) return;
   if (!CalculateVortex(2, prevViPlus, prevViMinus)) return;

   bool crossUp = (prevViPlus <= prevViMinus) && (viPlus > viMinus);

   if (crossUp && !HasOpenPosition())
     {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 1005)
        {
         return true;
        }
     }
   ...
   if(type == ORDER_TYPE_BUY)
     {
      ...
      trade.Buy(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "Vortex_005");
     }
  }
...
string GetStrategyName() { return "Vortex"; }
