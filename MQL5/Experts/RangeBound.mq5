...
// --- 1. PARAMETER ---
input int InpPeriod = 13;            // Range Periode
input double Threshold = 0.01;       // Threshold (Schwelle)
...
int OnInit()
  {
   trade.SetExpertMagicNumber(1018);
...
void OnTick()
  {
   ...
   if (rangeSize > 0)
     {
      positionInRange = (close - lowest) / rangeSize;
     }

   double atrVal = atr_buffer[0];
   bool isRangeQualityOk = atrVal < (rangeSize * 0.3);

   bool nearSupport = positionInRange < 0.3;
   bool momentumUp = close > close_buffer[1]; // close > Bars.ClosePrices.Last(2) in cTrader is equivalent to [0] > [1] in MT5

   if (nearSupport && momentumUp && isRangeQualityOk && !HasOpenPosition())
     {
   ...
   for(int i = PositionsTotal() - 1; i >= 0; i--)
     {
      ...
      if(PositionGetString(POSITION_SYMBOL) == _Symbol && PositionGetInteger(POSITION_MAGIC) == 1018)
        {
         return true;
        }
     }
   ...
   if(type == ORDER_TYPE_BUY)
     {
      ...
      trade.Buy(VolumeLots, _Symbol, currentPrice, slPrice, tpPrice, "RangeBound_018");
     }
  }
...
string GetStrategyName() { return "RangeBound"; }
